...
export function DesgloseReparto({ comprasMes, nombres }: Props) {
  const datos = useMemo(() => {
    let compartido = 0;
    let soloFranco = 0;
    let soloFabiola = 0;
    ...
    const total = compartido + soloFranco + soloFabiola + personalizado;
    return [
      { id: "compartido", nombre: "Compartido (50/50)", total: compartido, color: "#6366f1", icono: "🤝" },
      { id: "solo_franco", nombre: `Solo ${nombres.franco}`, total: soloFranco, color: "#3b82f6", icono: "👤" },
      { id: "solo_fabiola", nombre: `Solo ${nombres.fabiola}`, total: soloFabiola, color: "#10b981", icono: "👤" },
      { id: "personalizado", nombre: "Personalizado", total: personalizado, color: "#f59e0b", icono: "⚙️" },
